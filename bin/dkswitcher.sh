# !/bin/sh
# The following code is heavily based on NiLuJe's. Thanks!
...
toggle()
{
	if [ "${device_is_supported}" == "false" ] ; then
		# LOL
		eips_print_bottom_centered "Tested only on Kindle Touch, aborting." 0
		exit 1
	fi
	
	if !([ -f "/mnt/us/DUOKAN_DISABLE" ]) ; then
		mntroot rw
		touch /mnt/us/DUOKAN_DISABLE
		mntroot ro
		eips_print_bottom_centered "Duokan autostart disabled." 1
...
case "${1}" in
	...
	;;
	* )
		eips_print_bottom_centered "Invalid action (${1})" 0
		exit 1
	;;
esac
